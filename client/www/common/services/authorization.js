...
.factory('auth', ['$http','$window','$location', function($http , $window , $location){
	...
	{
		$window.localStorage['buddysms-app-token'] = token;
	};
	auth.getToken = function()
	{
		return $window.localStorage['buddysms-app-token'];
	};
	...
	auth.logOut = function(redirectTo)
	{
		//return $http.post('/logout').success(function(){
			$window.localStorage.removeItem('buddysms-app-token');
			redirect(redirectTo);
		//});
	};
